...
<table class="p-table">
	...
	<tbody>
		<?php foreach ( $templates as $template ) : ?>
			<?php
			$type          = get_post_meta( $template->ID, '_pedroea_template_type', true ) ?: 'header';
			...
			$is_active     = '1' === get_post_meta( $template->ID, '_pedroea_template_active', true );
			...
			$toggle_url    = wp_nonce_url( admin_url( 'admin-post.php?action=pedroea_toggle_active&post_id=' . $template->ID ), 'pedroea_toggle_' . $template->ID );
			$delete_url    = wp_nonce_url( admin_url( 'admin-post.php?action=pedroea_delete_template&post_id=' . $template->ID ), 'pedroea_delete_' . $template->ID );
			$edit_props    = admin_url( 'admin.php?page=pedroea_hfb&edit=' . $template->ID );
			...
			<tr>
				...
				<td><span class="pedroea-type-badge pedroea-type-badge-<?php echo esc_attr( $type ); ?>"><?php echo esc_html( ucfirst( $type ) ); ?></span></td>
